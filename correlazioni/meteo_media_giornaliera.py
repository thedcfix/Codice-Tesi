...
data = pd.read_csv('DATI_METEO_2015.csv',sep=',', decimal='.')

print("\nMerging...")
data = pd.merge(st, data, how='inner', on=['IdSensore', 'IdSensore'])

print("\nFiltering...")
data = data.loc[data.Tipologia == "Umidità Relativa"]
data = data.loc[data.Provincia == "MI"]
...
data.to_csv("DATA_TEMP_zz.csv", sep=';', decimal=',', header=True, index=False)